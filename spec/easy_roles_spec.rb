...
describe EasyRoles do
    ...
    describe "scopes" do
      describe "with_role" do
        it "should implement the `with_role` scope" do
          SerializeUser.respond_to?(:with_role).should be_truthy
        end

        it "should return an ActiveRecord::Relation" do
          expect(SerializeUser.with_role('admin').class.to_s).to eq('SerializeUser::ActiveRecord_Relation')
        end

        it "should match records for a given role" do
          user = SerializeUser.create(name: 'Daniel')
          SerializeUser.with_role('admin').include?(user).should be_falsey
          user.add_role! 'admin'
          SerializeUser.with_role('admin').include?(user).should be_truthy
        end
        ...
        it "should be chainable" do
          ...
          admin_users.include?(ryan).should be_truthy
          admin_users.include?(daniel).should be_falsey
        end
        ...
        it "should prove that wrapper markers are a necessary strategy by failing without them" do
          ...
          SerializeUser.with_role('recursion').include?(morgan).should be_truthy
        ...
        it "should avoid incorrectly matching roles where the name is a subset of another role's name" do
          ...
          (morgan = SerializeUser.create(name: 'Mr. Freeman')).add_role!('onrecursionrecursi')
          SerializeUser.with_role('recursion').include?(chuck).should be_truthy
          SerializeUser.with_role('recursion').include?(morgan).should be_falsey
        end
        ...
        it "should not allow roles to be added if they include the ROLES_MARKER character" do
          ...
          user.add_role!('funkytown!').should be_falsey
          SerializeUser::ROLES_MARKER = marker_cache
        end
        ...
        it "should correctly handle markers on failed saves" do
          ...
          imposter.roles.any? {|r| r.include?(SerializeUser::ROLES_MARKER) }.should be_falsey
        end
  ...
  describe "bitmask method" do
    it "should allow me to set a users role" do
      ...
      user.add_role 'admin'
      user._roles.include?("admin").should be_truthy
    end
    ...
    describe "scopes" do
      describe "with_role" do
        it "should implement the `with_role` scope" do
          BitmaskUser.respond_to?(:with_role).should be_truthy
        end

        it "should return an ActiveRecord::Relation" do
          BitmaskUser.with_role('admin').class.to_s.should == 'BitmaskUser::ActiveRecord_Relation'
        end
        ...
        it "should match records with a given role" do
          ...
          BitmaskUser.with_role('admin').include?(user).should be_falsey
          user.add_role! 'admin'
          BitmaskUser.with_role('admin').include?(user).should be_truthy
        end
